...
Here we're using the `js›xHeight` to get the height of the first line, and then the `js›leading` to get the height of the remaining lines with `js›leading * (numLines - 1)`.
