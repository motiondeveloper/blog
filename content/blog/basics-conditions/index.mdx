...
Now that you have learned how to [create and use variables](/blog/basics-variables), the next step is to make decisions based on their value.
...
- The `js›if` keyword
- The condition expression in parenthesis, that will resolve to `js›true` or `js›false`
...
For more info on JavaScript types, see the [Intro to Data Types](/blog/basics-intro-to-data-types) post
...
if (jobTitle === 'Animator') {
  ...
  // jobTitle isn't Animator or Developer
  'Working hard!';
}
```
...
Each side of a logical operator is a separate condition expression, so the expression `js›(software === 'VS Code' || 'After Effects')` will always be `js›true` as, `js›'After Effects'` resolves to `js›true`.
