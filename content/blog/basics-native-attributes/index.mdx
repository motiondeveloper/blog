...
The reason layer and property attributes work without the object reference is that After Effects **pre processes** your expression to add references to the current layer or property object on native attributes.
...
After Effects pre processes expressions, adding the appropriate object references to property and layer attributes.

</Inform>

This pre processing is not done to code in external files, so you must ensure to add the object references yourself in that case.

<Note>

Whether you add the property references yourself, or let After Effects add them during pre processing, is a matter or preference. For the sake of being explicit about what objects the attributes belong to, I prefer to add object references in my own expressions.
