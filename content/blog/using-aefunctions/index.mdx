...
To start using `js›aeFunctions` in an expression, you first need to create a reference to the file you imported earlier.

```js
const aef = footage('aefunctions.jsx').sourceData;
```
...
This variable is an [object](/blog/basics-objects), with each of the [functions](/blog/basics-functions) available as properties of the `js›aef` object.
