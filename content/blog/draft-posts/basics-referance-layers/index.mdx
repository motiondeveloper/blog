...
Each composition has built in properties, one of which is the `js›name` property is the name of the composition. We can use this to make sure we've selected the correct composition in our project.
...
In the same way that `js›comp()` is a built in [function](/blog/basics-functions) for referencing compositions, `js›layer()` is a function *built into composition objects* for referencing layers that takes.
...
Now you've learnt the first two parts of the expression created by the pick whip, referencing the composition and layer, you can move onto referencing the **layer attributes**, such as position, opacity and scale etc.
...
Each layer has properties that you can change 
