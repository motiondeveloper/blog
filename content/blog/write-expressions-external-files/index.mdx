...
- **Improve performance.**
  ...
  After effects pre-processes expressions to allow for ExtendScript backwards compatibility. This pre-processing isn't done on code in external files, so there's a performance improvement for long expressions when written in `js›.jsx` files.
...
Writing JavaScript in external files has some stricter requirements than writing within After Effects, as the pre-processing done on expressions isn't done the code in `js›.jsx` files.
